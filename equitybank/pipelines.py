...
class EequitybankPipeline:

    # Database setup
    conn = sqlite3.connect('equitybank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `equitybank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM equitybank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO equitybank (date, title, link, content)"
